...
export default function CartDrawer({ open, onOpenChange, items = [], onRemove, onQtyChange, onPlaceOrder, submitting }) {
  ...
  // Safe fallback against undefined/null state while calculating total
        ...
        {safeItems.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
              {safeItems.map((item) => (
                <div key={item.id || item.name} className="flex items-center gap-3 py-2 border-b last:border-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => item.quantity === 1 ? onRemove(item.id || item.name) : onQtyChange(item.name, item.quantity - 1)}
                      className="w-7 h-7 rounded-md border flex items-center justify-center text-red-500 hover:bg-red-50"
                    >
                      {item.quantity === 1 ? <Trash2 className="w-3.5 h-3.5" /> : <Minus className="w-3.5 h-3.5 text-foreground" />}
                    </button>
            ...
            <div className="px-5 py-3 border-t space-y-3">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <span className="text-xl font-heading font-bold text-blue-700">₵{total.toFixed(2)}</span>
              </div>
            </div>

            <SheetFooter className="px-5 pb-5 pt-1">
              <Button
                className="w-full bg-blue-500 hover:bg-blue-600 text-white rounded-lg h-11"
